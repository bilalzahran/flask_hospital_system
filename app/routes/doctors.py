...
from app.service.doctors import delete_doctor, get_all_doctors, add_doctor, get_doctor, update_doctor
...
doctor = doctor_routes.model(
  "doctor",
  {
    ...
    "work_start_time": fields.datetime(required=True),
    "work_end_time": fields.datetime(required=True)
  }
)
...
class Doctor(Resource):
  ...
  def put(self, employee_id):
      data = request.json
      employee = get_doctor(employee_id)
      if employee:
          return update_doctor(employee_id, data)
      else:
          doctor_routes.abort(404)
  ...
  def delete(self, employee_id):
      employee = get_doctor(employee_id)
      if employee:
          return delete_doctor(employee_id)
      else:
          doctor_routes.abort(404)
